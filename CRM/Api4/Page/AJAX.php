  ...
  public function run() {
    try {
      ...
      if (empty($this->urlPath[3])) {
        $calls    = CRM_Utils_Request::retrieve('calls', 'String',
          CRM_Core_DAO::$_nullObject, TRUE, NULL, 'REQUEST', TRUE);
        $calls    = json_decode($calls, TRUE);
        $response = [];
        foreach ($calls as $index => $call) {
          $response[$index] = $this->execute($call[0], $call[1],
            CRM_Utils_Array::value(2, $call, []));
        }
      } // Call single
      else {
